...
Готовый сценарий поднять распределённый Hadoop+Spark-кластер на **трёх Windows-ноутбуках** через Docker. Один и тот же скрипт `start.bat` на каждой машине определяет её роль по IP и поднимает нужный набор контейнеров.
...
**Открыть порты в Windows Firewall** (на каждом ноуте):
...
1. IP в `cluster.conf` совпадают с реальными (`ipconfig` на каждом ноуте).
...
Учебный проект. Apache Hadoop и Apache Spark распространяются под Apache License 2.0.
